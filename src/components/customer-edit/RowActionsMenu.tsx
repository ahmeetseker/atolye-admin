...
interface RowActionsMenuProps {
  onEdit: () => void
  onDelete: () => void
  /** ID used to compose the data-testid on the trigger (e.g. `M-2401`). */
  rowId: string
...
/**
 * Customer row action menu (⋮ — Düzenle / Sil).
 *
 * Wave F1: this is intentionally a copy of the listings sibling under
 * `components/listing-edit/RowActionMenu.tsx`. The two parallel agents (F1A
 * + F1B) keep their own files; a follow-up wave (F2) can DRY them into a
 * shared component once both stabilise.
 *
 * - 44px touch target on mobile (h-11 w-11), 32px on md+ for table density.
 * - Keyboard: ↑/↓ moves focus, Enter activates, Esc closes and restores
 *   focus to the trigger.
 * - Click outside closes.
 */
export function RowActionsMenu({ onEdit, onDelete, rowId, label = 'Müşteri işlemleri' }: RowActionsMenuProps) {
  ...
  return (
    <div className="relative inline-flex" onClick={(e) => e.stopPropagation()}>
      <button
        ...
        aria-label={label}
        data-testid={`customer-row-actions-${rowId}`}
        onClick={(e) => {
          e.stopPropagation()
          setOpen((v) => !v)
          setActive('edit')
      ...
      {open && (
        ...
        >
          <button
            ...
            type="button"
            role="menuitem"
            data-testid="customer-row-action-edit"
