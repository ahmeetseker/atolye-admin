...
interface DeleteEventDialogProps {
  open: boolean
  /** Event id used in the confirm message when no title is available. */
  id?: string
  /** Optional event title rendered inside the confirm sentence. */
  eventTitle?: string
  pending?: boolean
  error?: Error | null
  onConfirm: () => void
...
/**
 * Destructive confirm dialog for a calendar event.
 *
...
export function DeleteEventDialog({
  ...
  useEffect(() => {
    ...
    if (wasPending && !pending && confirmedRef.current) {
      ...
      if (error) {
        ...
        toast('Etkinlik silindi', { variant: 'success' })
      }
    }
  ...
  const subject = eventTitle ?? (id ? id : 'bu')
  ...
  return (
    <AnimatePresence>
      {open && (
        <div
          role="presentation"
          data-testid="event-delete-dialog"
        ...
        >
          ...
          <motion.div
            ...
            aria-describedby="event-delete-desc"
          ...
          >
            <div className="flex items-start gap-3 p-4 md:p-6">
              ...
              <div className="min-w-0 flex-1">
                ...
                >
                  Etkinlik <em className="font-serif italic font-light">silinsin mi</em>?
                </h2>
                <p
                  id="event-delete-desc"
                  className="mt-1.5 text-[13.5px] leading-relaxed text-muted-foreground"
                >
                  <span className="text-foreground">{subject}</span> etkinliğini
