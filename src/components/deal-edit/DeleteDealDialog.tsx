...
/**
 * Destructive-confirm dialog for a sales deal. Mirrors DeleteListingDialog
 * conventions: token-only, Esc + backdrop close, initial focus on İptal.
 */
export function DeleteDealDialog({
  ...
  useEffect(() => {
    ...
    if (wasPending && !pending && confirmedRef.current) {
      ...
      if (error) {
        toast(`Silinemedi: ${error.message || 'Tekrar dener misin?'}`, {
          variant: 'error',
        })
      } else {
  ...
  const subject = customerName ?? (id ? id : 'bu')
  ...
  return (
    <AnimatePresence>
      {open && (
        <div
          role="presentation"
          data-testid="deal-delete-dialog"
        ...
        >
          ...
          <motion.div
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="deal-delete-title"
            aria-describedby="deal-delete-desc"
          ...
          >
            <div className="flex items-start gap-3 p-4 md:p-6">
              ...
              <div className="min-w-0 flex-1">
                <h2
                  id="deal-delete-title"
                  className="font-serif text-lg font-light leading-tight"
                >
                  Fırsat <em className="font-serif italic font-light">silinsin mi</em>?
                </h2>
                <p
                  id="deal-delete-desc"
                  className="mt-1.5 text-[13.5px] leading-relaxed text-muted-foreground"
                >
                  <span className="text-foreground">{subject}</span> fırsatını silmek
                  istediğinizden emin misiniz? Bu işlem{' '}
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-border bg-muted/30 px-4 py-3 md:flex-row md:items-center md:justify-end md:px-6">
              <button
                ref={cancelRef}
                type="button"
                data-testid="deal-delete-cancel"
                onClick={onCancel}
                disabled={pending}
                className={cn(
